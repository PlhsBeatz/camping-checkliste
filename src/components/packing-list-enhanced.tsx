...
import {
  classifyBulkDeleteSemantics,
  collectBulkScopeTravelers,
  computeBulkEditFieldDefaults,
  getBulkEditBemerkungMode,
  needsBulkPersonSelection,
...
const PackingItem: React.FC<PackingItemProps> = ({
  ...
  const usesGruppenCheckboxes =
    ...
    (pauschalModus === 'pro_gruppe' ||
      pauschalModus === 'ausgewaehlte_gruppen' ||
      (fullItem.gruppen ?? []).length > 0);
  ...
  const showGruppenChipsRow =
    multiGroupActive && mitreisenden_typ === 'pauschal' && pauschalGruppenFilter === 'alle';
  const allVacationGruppeIds = useMemo(() => {
    if (vacationGroups) {
      const ids: string[] = [];
      if (vacationGroups.ownGroup.length > 0 && ownGruppeId) {
        ids.push(ownGruppeId);
      }
      for (const g of vacationGroups.otherGroups) {
    ...
    return (fullItem.gruppen ?? []).map((g) => g.gruppe_id);
  }, [vacationGroups, ownGruppeId, fullItem.gruppen]);
  const assignedGruppeIds = useMemo(
    () =>
      getSelectedGruppeIdsFromAssignment(
        pauschalModus,
        fullItem.verantwortliche_gruppe_id,
        (fullItem.gruppen ?? []).map((g) => g.gruppe_id),
        allVacationGruppeIds
      ),
    [
      pauschalModus,
  ...
  const assignedGroupsForChipsRow = useMemo(() => {
    const nameById = new Map<string, string>();
    if (vacationGroups) {
      if (ownGruppeId) {
        nameById.set(ownGruppeId, gruppenMap.get(ownGruppeId) ?? 'Mein Haushalt');
      }
      for (const g of vacationGroups.otherGroups) {
        if (g.members.length > 0) {
    ...
    for (const g of fullItem.gruppen ?? []) {
      nameById.set(
        g.gruppe_id,
        resolveGruppeName(g.gruppe_id, gruppenMap, g.gruppe_name)
      );
    }
    if (fullItem.verantwortliche_gruppe_id) {
      nameById.set(
        fullItem.verantwortliche_gruppe_id,
        resolveGruppeName(
          fullItem.verantwortliche_gruppe_id,
          gruppenMap,
          fullItem.verantwortliche_gruppe_name
        )
      );
    }
    return assignedGruppeIds.map((id) => ({
      id,
  ...
  const handleGruppenMainToggle = () => {
    ...
    if (hidePackedItems && wasUnpacked && onShowToast) {
      const nextGruppen = (fullItem.gruppen ?? []).map((g) => {
        if (g.gruppe_id !== toggleGruppeId) return g;
        if (canConfirmVorgemerkt) {
          return { ...g, gepackt: true, gepackt_vorgemerkt: false };
        }
        return { ...g, gepackt: false, gepackt_vorgemerkt: true };
      });
  ...
  const handlePauschalToggle = () => {
    ...
    if (
      ...
      toggleGruppeId &&
      toggleGruppeEntry &&
      canToggleOwnGruppeMain
    ) {
      handleGruppenMainToggle();
      return;
    }
      ...
      >
        ...
        <div className="flex items-start space-x-3">
          {showBulkSelectCheckbox && (
            <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center" data-no-bulk-select>
              <button
                type="button"
              ...
              >
                {bulkSelected ? (
                  ...
                  <span className="h-2 w-2 rounded-full bg-[rgb(45,79,30)]/30" aria-hidden />
                )}
              </button>
            </div>
          )}
          ...
          <div className="flex-grow min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {multiGroupActive &&
                ...
                <PauschalGruppeBadge
                  ...
                  onClick={
                    canEditPauschalEntries && onOpenAssignment
                      ? () => onOpenAssignment(fullItem)
                      : undefined
                  }
                />
              )}
            ...
            {showGruppenChipsRow && assignedGroupsForChipsRow.length > 0 && (
              <div className="mt-1.5 flex flex-wrap gap-x-1 gap-y-1">
                {assignedGroupsForChipsRow.map((g) => {
                  const gruppeEntry = (fullItem.gruppen ?? []).find(
                    (entry) => entry.gruppe_id === g.id
                  );
                  const showPackStatus =
                    usesGruppenCheckboxes || pauschalModus === 'einmal';
                  const packed = gruppeEntry
                    ? canConfirmVorgemerkt
                      ? gruppeEntry.gepackt
                      : gruppeEntry.gepackt || !!gruppeEntry.gepackt_vorgemerkt
                    : pauschalModus === 'einmal'
                      ? canConfirmVorgemerkt
                        ? gepackt
                        : effectivePacked(gepackt, gepackt_vorgemerkt)
                      : false;
                  const vorgemerkt = gruppeEntry
                    ? !!gruppeEntry.gepackt_vorgemerkt && !gruppeEntry.gepackt
                    : pauschalModus === 'einmal'
                  ...
                  const chipClassName = cn(
                    'inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-[10px] font-medium transition-colors shrink-0',
                    g.id === ownGruppeId
                      ? 'border-[rgb(45,79,30)]/50 bg-[rgb(45,79,30)]/5 text-[rgb(45,79,30)] dark:text-green-300 font-semibold'
                      : 'border-[rgb(45,79,30)]/40 bg-[rgb(45,79,30)]/10 text-[rgb(45,79,30)] dark:text-green-300',
                    canOpenAssignment && 'cursor-pointer hover:bg-muted/50 active:scale-[0.98]'
                  );
                  const chipContent = (
                    <>
                      {showPackStatus && (
                  ...
                  if (canOpenAssignment) {
                    return (
                      <button
                        ...
                        type="button"
                        title={g.name}
                        onClick={() => onOpenAssignment(fullItem)}
                  ...
                  return (
                    <span key={g.id} className={chipClassName} title={g.name}>
                      {chipContent}
                    </span>
                  );
                })}
...
export function PackingList({
  ...
  const [bulkPersonsDialog, setBulkPersonsDialog] = useState<{
    mode: 'edit' | 'delete';
    travelers: TravelerForMarkAll[];
  } | null>(null);
  const [bulkSelectedPersonIds, setBulkSelectedPersonIds] = useState<string[] | null>(null);
  ...
  const allVacationGruppeIdsForList = useMemo(
    () => getVacationGruppeIds(vacationMitreisende),
    [vacationMitreisende]
  );
  const vacationGroupsForSheet = useMemo(() => {
    ...
    if (profileGroups.ownGroup.length > 0 && ownGruppeId) {
      groups.push({ id: ownGruppeId, name: gruppenMap.get(ownGruppeId) ?? 'Mein Haushalt', members: profileGroups.ownGroup });
    }
  ...
  const isBulkAssignableItem = useCallback(
    (item: DBPackingItem) =>
      multiGroupActive &&
      canEditPauschalEntries &&
      item.mitreisenden_typ === 'pauschal',
  ...
  const allSelectedBulkAssignable = useMemo(
    () =>
      bulkSelectedIds.size > 0 &&
      bulkSelectedItems.every((item) => isBulkAssignableItem(item)),
    [bulkSelectedIds.size, bulkSelectedItems, isBulkAssignableItem]
  );

  const packProfileScopeIdSet = useMemo(() => {
    const scope = packProfileScopeMitreisende ?? vacationMitreisende;
    return new Set(scope.map((m) => m.id));
  }, [packProfileScopeMitreisende, vacationMitreisende]);

  const bulkNeedsPersonPicker = useMemo(
    () => needsBulkPersonSelection(bulkSelectedItems, selectedProfile),
    [bulkSelectedItems, selectedProfile]
  );

  const bulkScopeTravelers = useMemo(
    () =>
      collectBulkScopeTravelers(
        bulkSelectedItems,
    ...
    [bulkSelectedItems, packProfileScopeIdSet, packProfileScopeMitreisende, vacationMitreisende]
  );

  const bulkEditFieldDefaults = useMemo(
    () =>
      computeBulkEditFieldDefaults(
        bulkSelectedItems,
        selectedProfile,
        packProfileScopeIdSet,
        bulkSelectedPersonIds
      ),
    [bulkSelectedItems, selectedProfile, packProfileScopeIdSet, bulkSelectedPersonIds]
  );

  const bulkEditBemerkungMode = useMemo(
    () =>
      getBulkEditBemerkungMode(
        bulkSelectedItems,
        selectedProfile,
  ...
  const bulkDeleteSemantics = useMemo(
    () =>
      classifyBulkDeleteSemantics(
        bulkSelectedItems,
        selectedProfile,
  ...
  const exitBulkSelection = useCallback(() => {
    ...
    setBulkPersonsDialog(null);
    setBulkSelectedPersonIds(null);
  }, []);
  ...
  const startBulkEditFlow = useCallback(() => {
    if (bulkSelectedIds.size === 0) return;
    const ids = [...bulkSelectedIds];
    const openEdit = () => {
      setBulkSelectedPersonIds(null);
      if (bulkNeedsPersonPicker && bulkScopeTravelers.length > 0) {
        setBulkPersonsDialog({ mode: 'edit', travelers: bulkScopeTravelers });
        return;
      }
      setBulkEditOpen(true);
    };
    if (beforeBulkEdit) beforeBulkEdit(ids, openEdit);
    else openEdit();
  }, [bulkSelectedIds, bulkNeedsPersonPicker, bulkScopeTravelers, beforeBulkEdit]);

  const startBulkDeleteFlow = useCallback(() => {
    if (bulkSelectedIds.size === 0) return;
    const ids = [...bulkSelectedIds];
    const openDelete = () => {
      setBulkSelectedPersonIds(null);
      if (bulkNeedsPersonPicker && bulkScopeTravelers.length > 0) {
        setBulkPersonsDialog({ mode: 'delete', travelers: bulkScopeTravelers });
        return;
      }
      setBulkDeleteOpen(true);
    };
    if (beforeBulkDelete) beforeBulkDelete(ids, openDelete);
    else openDelete();
  }, [bulkSelectedIds, bulkNeedsPersonPicker, bulkScopeTravelers, beforeBulkDelete]);

  const handleBulkPersonsConfirm = useCallback(
    (selectedTravelerIds: string[]) => {
      if (!bulkPersonsDialog) return;
      setBulkSelectedPersonIds(selectedTravelerIds);
      const mode = bulkPersonsDialog.mode;
      setBulkPersonsDialog(null);
      if (mode === 'edit') setBulkEditOpen(true);
  ...
  useEffect(() => {
    onBulkSelectionModeChange?.(bulkSelectionMode);
  }, [bulkSelectionMode, onBulkSelectionModeChange]);
  ...
  return (
    <Tabs value={activeMainCategory} onValueChange={setActiveMainCategory} className="flex flex-col flex-1 min-h-0 min-w-0 w-full max-w-full">
                      ...
                      <CardContent className="p-0 bg-transparent">
                        {categoryItems.map(item => (
                            <PackingItem
                              ...
                              bulkSelectable={showBulkSelectionUi}
                              onBulkSelectionToggle={
                                showBulkSelectionUi ? () => toggleBulkSelection(item.id) : undefined
                              }
                              onBulkSelectionStart={
      ...
      {bulkSelectionMode && showBulkSelectionUi && (
        <BulkSelectionBar
          count={bulkSelectedIds.size}
          onCancel={exitBulkSelection}
          onAssign={
            showBulkAssignUi && allSelectedBulkAssignable
              ? () => {
                  if (bulkSelectedIds.size === 0) return;
                  const ids = [...bulkSelectedIds];
                  const openAssign = () => setBulkAssignOpen(true);
                  if (beforeBulkAssign) beforeBulkAssign(ids, openAssign);
                  else openAssign();
                }
              : undefined
          }
          onEdit={
            onBulkUpdatePackingItems
              ? () => {
                  if (bulkSelectedIds.size > 0) startBulkEditFlow();
                }
              : undefined
          }
          onDelete={
            onBulkDeletePackingItems
              ? () => {
                  if (bulkSelectedIds.size > 0) startBulkDeleteFlow();
                }
              : undefined
          }
        />
      ...
      {onBulkUpdatePackingItems && (
        <BulkPackingEditModal
          ...
          onConfirm={async (patch) => {
            const ids = [...bulkSelectedIds];
            const ok = await onBulkUpdatePackingItems(
              ids,
              patch,
