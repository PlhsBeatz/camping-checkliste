 ...
 * Batch-Loading für Mitreisende (kein N+1)
 */
  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      const mitResult = await db
        ...
        .bind(...ids)
        .all<{ packlisten_eintrag_id: string; mitreisender_id: string; mitreisender_name: string; gepackt: number; anzahl?: number | null }>()
